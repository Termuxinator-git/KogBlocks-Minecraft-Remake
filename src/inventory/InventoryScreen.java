...
public class InventoryScreen {
    private Texture inventoryTex;
    private SteveRenderer steve;
    ...
    private static final float GUI_OFFSET_X = -5f;
    private static final float GUI_OFFSET_Y = -15f;
    private static final float GRID_OFFSET_Y = -9.5f;
    ...
    private static final float CRAFT_GRID_OFFSET_X = 20f;
    private static final float CRAFT_GRID_OFFSET_Y = -10f;
    private static final float CRAFT_RESULT_OFFSET_X = 20f;
    private static final float CRAFT_RESULT_OFFSET_Y = -10f;

    private int heldItem = Chunk.AIR;
    private int heldCount = 0;

    private int[][] craftGrid = new int[2][2];
    private int[][] craftCounts = new int[2][2];
    private int craftResult = Chunk.AIR;
    private int craftResultCount = 0;

    private boolean rightDragging = false;
    private final boolean[] dragVisited = new boolean[Inventory.SIZE + 4];

    private double lastLeftClickTime = 0.0;
    private int lastLeftClickSlot = -1;

    public void init() {
        inventoryTex = new Texture("textures/gui/inventory.png");
    }

    public void render(Hotbar hotbar) {
        float baseX = (W - WIDTH) / 2f;
        float baseY = (H - HEIGHT) / 2f;

        float drawX = baseX + GUI_OFFSET_X;
        float drawY = baseY + GUI_OFFSET_Y;
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                ...
                float slotY = drawY + 84f * SCALE + GRID_OFFSET_Y + row * SLOT_STEP - 0.5f * SCALE;
        ...
        for (int col = 0; col < 9; col++) {
            ...
            float slotX = drawX + 8f * SCALE + col * SLOT_STEP;
            float slotY = drawY + 142f * SCALE + GRID_OFFSET_Y - 0.5f * SCALE;
        ...
        for (int row = 0; row < 2; row++) {
            for (int col = 0; col < 2; col++) {
                ...
                float slotX = drawX + 88f * SCALE + CRAFT_GRID_OFFSET_X + col * SLOT_STEP;
                float slotY = drawY + 18f * SCALE + CRAFT_GRID_OFFSET_Y + row * SLOT_STEP;
        ...
        if (craftResult != Chunk.AIR) {
            float resultX = drawX + 144f * SCALE + CRAFT_RESULT_OFFSET_X;
            float resultY = drawY + 28f * SCALE + CRAFT_RESULT_OFFSET_Y;
        ...
        if (steve != null) {
            ...
            glTranslatef(drawX + 100f, drawY + 144f, -200f);
            glScalef(3f, -3f, 3f);

            glRotatef((mx - (drawX + 51f)) * 0.08f, 0f, 1f, 0f);
            glRotatef((my - (drawY + 75f)) * 0.05f, 1f, 0f, 0f);

            steve.render(0f, 0f, 0f, 0f);
    ...
    public void handleInput(Hotbar hotbar) {
        int craftSlot = getCraftSlotAt((float) Input.getMouseX(), (float) Input.getMouseY());
        boolean overResult = isOverCraftResult((float) Input.getMouseX(), (float) Input.getMouseY());

        Inventory inv = hotbar.getInventory();
        int hoveredSlot = getSlotAt((float) Input.getMouseX(), (float) Input.getMouseY());
        ...
        if (Input.isMouseJustPressed(GLFW_MOUSE_BUTTON_RIGHT)) {
            ...
            if (craftSlot != -1) {
                ...
                if (rightDragging) {
                    dragVisited[Inventory.SIZE + row * 2 + col] = true;
                }
                return;
            }
    ...
    private void updateCrafting() {
        ...
        if (isStickRecipeLeft() || isStickRecipeRight()) {
        ...
        for (int r = 0; r < 2; r++) {
            for (int c = 0; c < 2; c++) {
                if (craftGrid[r][c] == Chunk.OAK_LOG) {
                    totalLogs += craftCounts[r][c];
                }
            }
        }
    ...
    private int getCraftSlotAt(float mx, float my) {
        ...
        for (int r = 0; r < 2; r++) {
            for (int c = 0; c < 2; c++) {
                float x = drawX + 88f * SCALE + CRAFT_GRID_OFFSET_X + c * SLOT_STEP;
                float y = drawY + 18f * SCALE + CRAFT_GRID_OFFSET_Y + r * SLOT_STEP;
    ...
    private boolean isOverCraftResult(float mx, float my) {
        ...
        float x = drawX + 144f * SCALE + CRAFT_RESULT_OFFSET_X;
        float y = drawY + 28f * SCALE + CRAFT_RESULT_OFFSET_Y;
    ...
    private int getSlotAt(float mx, float my) {
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                ...
                float y = drawY + 84f * SCALE + row * SLOT_STEP;
    ...
    public void setSteveRenderer(SteveRenderer steve) {
        this.steve = steve;
    }
    ...
    private void handleCraftRightDrag(int row, int col) {
        int dragIndex = Inventory.SIZE + row * 2 + col;
        if (heldItem == Chunk.AIR || heldCount <= 0) return;
    ...
    private boolean isStickRecipeLeft() {
        return craftGrid[0][0] == Chunk.OAK_PLANKS &&
               craftGrid[1][0] == Chunk.OAK_PLANKS &&
               craftCounts[0][0] > 0 &&
               craftCounts[1][0] > 0 &&
               craftGrid[0][1] == Chunk.AIR &&
               craftGrid[1][1] == Chunk.AIR;
    }

    private boolean isStickRecipeRight() {
        return craftGrid[0][1] == Chunk.OAK_PLANKS &&
               craftGrid[1][1] == Chunk.OAK_PLANKS &&
               craftCounts[0][1] > 0 &&
               craftCounts[1][1] > 0 &&
               craftGrid[0][0] == Chunk.AIR &&
               craftGrid[1][0] == Chunk.AIR;
    }

    private boolean isCraftingTableRecipe() {
        return craftGrid[0][0] == Chunk.OAK_PLANKS &&
    ...
    private void consumeCurrentCraftRecipe() {
        ...
        if (isStickRecipeLeft()) {
            consumeCraftSlot(0, 0, 1);
            consumeCraftSlot(1, 0, 1);
            return;
        }

        if (isStickRecipeRight()) {
            consumeCraftSlot(0, 1, 1);
            consumeCraftSlot(1, 1, 1);
            return;
        }
